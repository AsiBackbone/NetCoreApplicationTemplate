...
## Contract vectors for external adapters

NCAT publishes versioned, machine-readable contract vectors for the audit-completion handoff at [`contracts/audit-completion/v1/audit-completion-vectors.json`](https://github.com/AsiBackbone/NetCoreApplicationTemplate/blob/main/contracts/audit-completion/v1/audit-completion-vectors.json). External adapters use them to confirm that their own models still agree with NCAT without taking NCAT as a compile-time dependency. The vectors cover:

- representative receipts and dispatched completion messages, including null optional identifiers;
- canonical manifest text, its UTF-8 byte length, and the expected SHA-256 digest;
- idempotency key derivation and destinations;
- the supported `Committed` outcome, and the no-mutation, failed, and rolled-back cases that produce no message;
- invalid messages an adapter must reject.

The vectors are generated by production NCAT builders, the hasher, outbox staging, and dispatch. A contract test fails when production behavior drifts from the committed file. Pin a release tag or commit SHA when you consume the file; the first release to contain it is the one after `v2.10.0`.

The [contract README](https://github.com/AsiBackbone/NetCoreApplicationTemplate/blob/main/contracts/audit-completion/README.md) documents the fixture structure, how to consume it, and the compatibility policy for additive fields, breaking field changes, schema versions, and canonical hash changes.
