...
The vectors are generated by production NCAT builders, the hasher, outbox staging, and dispatch. A contract test fails when production behavior drifts from the committed file. Pin a release tag or commit SHA when you consume the file; the first release to contain it is `v2.11.0`.
